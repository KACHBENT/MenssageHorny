...
@php
    ...
    $groupMainAvatarUrl = $group->image
        ? asset('storage/' . ltrim($group->image, '/'))
        : 'https://ui-avatars.com/api/?name=' . urlencode($group->name) . '&size=40&background=6c757d&color=fff';

    $lastInitialMessageId = optional($messages->last())->id ?? 0;
@endphp
                    ...
                    <ul class="dropdown-menu dropdown-menu-end">
                        ...
                        <li>
                            <a class="dropdown-item align-items-center" href="{{ route('settingfaces.settingfaceprofile') }}" style="display:flex; gap:10px;">
                                <img src="{{ asset('image/icons/ar_on_you.svg') }}" alt="settingFaces" width="20" class="black-filter">Acceso biométricos
                            </a>
                        </li>
                        <li><hr class="dropdown-divider"></li>
                        <li>
            ...
            <div class="chat-list flex-grow-1 overflow-auto" id="chatsList">
                ...
                @forelse($groups ?? [] as $groupItem)
                    @php
                        ...
                        $activeGroup = (int) $groupItem->id === (int) $group->id ? 'active' : '';
        ...
        <section class="chat-area chat-main">
            <div class="chat-header">
                <div class="d-flex align-items-center min-w-0 w-100">
                    ...
                    <div class="position-relative me-3 flex-shrink-0">
                        <img src="{{ $groupMainAvatarUrl }}"
                             class="avatar"
                             alt="Grupo">
                    </div>

                    <div class="min-w-0">
                        <h6 class="mb-0 fw-semibold text-truncate">{{ $group->name }}</h6>
                        <small class="text-muted d-block text-truncate">
                            {{ $group->users->count() }} miembros ·
                            {{ $group->users->pluck('name')->take(4)->implode(', ') }}
                            @if($group->users->count() > 4) ... @endif
                        </small>
                    </div>
                </div>
            </div>
            ...
            <div class="messages-container" id="messagesContainer">
                @foreach($messages as $msg)
                    @php
                        $fileUrl = $msg->file_path ? asset('storage/' . ltrim($msg->file_path, '/')) : null;
                    ...
                    <div class="message {{ $isMine ? 'sent' : 'received' }}" data-message-id="{{ $msg->id }}">
                        <div class="message-content">
                            @if(!$isMine)
                                <div class="small fw-semibold mb-1 text-primary">{{ $msg->user->name ?? 'Usuario' }}</div>
                            @endif
...
<input type="hidden" id="groupSendMessageUrl" value="{{ route('groups.messages.send', $group->id) }}">
<input type="hidden" id="groupSendFileUrl" value="{{ route('groups.messages.send-file', $group->id) }}">
<input type="hidden" id="groupMessagesSinceUrl" value="{{ route('groups.messages.since', [$group->id, '__LAST__']) }}">
...
$(document).ready(function() {
    ...
    const groupSendMessageUrl = $('#groupSendMessageUrl').val();
    const groupSendFileUrl = $('#groupSendFileUrl').val();
    const groupMessagesSinceUrl = $('#groupMessagesSinceUrl').val();
    ...
    function sendMessage() {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                attachBtn.prop('disabled', false);
                showToast('Error al enviar mensaje al grupo', 'error');
            }
        });
    }
    ...
    function sendFile(e) {
        ...
        $.ajax({
            url: groupSendFileUrl,
            method: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            success: function(response) {
                attachBtn.html('<i class="fas fa-plus-circle"></i>').prop('disabled', false);
                ...
                if (response.success && response.data) {
                    appendMessage(response.data);
                    lastMessageId = Math.max(lastMessageId, parseInt(response.data.id || 0));
                    showToast('Archivo enviado al grupo', 'success');
                } else {
    ...
    function checkNewMessages() {
        $.ajax({
            url: groupMessagesSinceUrl.replace('__LAST__', lastMessageId),
            method: 'GET',
            success: function(messages) {
                if (!messages || !messages.length) return;

                messages.forEach(msg => {
                    if ($(`.message[data-message-id="${msg.id}"]`).length) return;
                    appendMessage(msg);
                    lastMessageId = Math.max(lastMessageId, parseInt(msg.id || 0));
                });
            },
            error: function(xhr) {
                console.error('Polling group error:', xhr.responseText);
            }
        });
    }
    ...
    function appendMessage(message) {
        ...
        if ($(`.message[data-message-id="${message.id}"]`).length) return;

        const type = parseInt(message.user_id) === currentUserId ? 'sent' : 'received';
        const senderName = parseInt(message.user_id) === currentUserId
            ? ''
            : `<div class="small fw-semibold mb-1 text-primary">${escapeHtml(message.user_name || 'Usuario')}</div>`;
    ...
    function renderMessageContent(message) {
        ...
        if (message.type === 'image') {
            ...
            return `
                <div class="image-preview-container">
                    <img src="${imageUrl}" class="img-fluid rounded chat-media-image" onclick="openImageModal('${imageUrl}')" alt="Imagen">
